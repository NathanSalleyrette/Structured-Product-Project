...
#include <chrono>
    ...
    Napi::Value MyPnlObject::ComputeDeltas(const Napi::CallbackInfo& info) {
        Napi::Env env = info.Env();
        ...
        int initialWindow = info[2].As<Napi::Number>().Int32Value();

        string refDate = info[3].ToString(); // Date vers laquelle on veut créer notre fenêtre
        //string refDate = "2021-12-10"; // Date vers laquelle on veut créer notre fenêtre

        string endFinishDate = info[4].ToString(); // Date jusqu'à laquelle on récupère les données historiques
        // Ne pas pas être supérieur à Aujourd'hui - 2 jours 
    ...
    //On a maintenant la part fixe du div ( rd - rf)
        ...
        for(int i = 0; i < delta->size; i++) deltasStock.push_back(GET(delta, i));

        stocksString.clear();

        for(int i = 0; i < market->actions.size(); i++) stocksString.push_back(market->actions[i]);
    ...
    Napi::Value MyPnlObject::Stocks(const Napi::CallbackInfo& info) {
        
        Napi::Env env = info.Env();       
        Napi::Array outputArray = Napi::Array::New(env, stocksString.size());

        for (int i = 0; i < stocksString.size(); i++) {
            outputArray[i] = Napi::String::New(env, stocksString[i]);

        }
        return outputArray;
    }

    Napi::Value MyPnlObject::Deltas(const Napi::CallbackInfo& info) {
